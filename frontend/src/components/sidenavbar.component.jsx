...
const SideNav = () => {
    ...
    //         // Check if pageStateTab.current is defined
    //         // Call the function directly instead of simulating a click event
    //         changePageState({ target: pageStateTab.current });
    //     }
    // }, [pageState]);
    useEffect(() => {
        setShowSideNav(false);
        ...
        if (pageStateTab.current && pageStateTab.current.click) {
            // Check if pageStateTab.current is defined and has a click method
            pageStateTab.current.click();
        }
    }, [pageState]);
    ...
    <>
        <section className='relative flex gap-10 py-0 m-0 max-md:flex-col'>
            ...
            <div className='sticky top-[80px] z-30'>
                ...
                <div className='md:hiiden bg-white py-1 border-b border-grey flex flex-nowrap overflow-x-auto '>
                    <button ref={sideBarIconTab} className='p-5 capitalize' onClick={changePageState}>
                    <i className='fi fi-rr-bars-staggered pointer-events-nones'></i>
                    </button>
